...
export default function CaregiverSignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // TODO: API 연동
      const res = await apiClient.post(ENDPOINTS.SIGNUP, { ...form, role: 'GUARDIAN' });
      if (res.success) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 py-10">
      <div className="w-full max-w-md bg-white rounded-2xl border-2 border-gray-200 p-8 shadow-sm">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-6">보호자 회원가입</h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <Field label="닉네임">
            <input type="text" required placeholder="환자 검색 시 사용됩니다"
