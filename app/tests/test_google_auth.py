...
async def test_google_register_rejects_soft_deleted_email(client: AsyncClient):
    ...
    with patch("app.api.google_auth.get_state_redis", return_value=mock_redis):
        resp = await client.post(
            "/api/auth/google/register",
            json={**_REGISTER_BASE, "email": "deleted@reg.com", "nickname": "삭제후가입"},
        )
...
@pytest.mark.asyncio
async def test_google_register_sanitizes_nickname(client: AsyncClient):
    """닉네임에 공백/특수문자 포함 → strip + 특수문자 제거 후 저장."""
    pending = _make_pending(google_id="google-sub-sanitize", email="sanitize@test.com")
    mock_redis = AsyncMock()
    mock_redis.get = AsyncMock(return_value=pending)
    mock_redis.delete = AsyncMock()
    ...
    with patch("app.api.google_auth.get_state_redis", return_value=mock_redis):
        resp = await client.post(
            "/api/auth/google/register",
            json={**_REGISTER_BASE, "email": "sanitize@test.com", "nickname": " 홍길동@! "},
        )
